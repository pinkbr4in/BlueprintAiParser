...
from .utils import (
    ...
    MACRO_PATH_REGEX, FUNCTION_REF_REGEX, CLASS_PATH_REGEX
)
...
class BlueprintParser:
    ...
    # !!! REMOVED get_pin_by_id and get_node_for_pin as they relied on the unreliable central map !!!
    # def get_pin_by_id(self, pin_id: str) -> Optional[Pin]:
    #     return self.pins_by_id.get(pin_id)
    # def get_node_for_pin(self, pin_id: str) -> Optional[Node]:
    #     pin = self.get_pin_by_id(pin_id)
    #     return self.get_node_by_guid(pin.node_guid) if pin and pin.node_guid else None
    ...
    def _process_and_finalize_node(self, node_data: Dict[str, Any]):
        ...
        # if ENABLE_PARSER_DEBUG: print(f"DEBUG: Finalized Node. Name='{name}', TempGUID='{temp_guid_used_for_creation}', FinalGUID='{final_guid}'", file=sys.stderr)

        # Update node_guid for all pins belonging to this node
        for pin in new_node.pins.values():
            if pin.node_guid != final_guid:
                 # if ENABLE_PARSER_DEBUG: print(f"DEBUG:   Updating Pin '{pin.name}' ({pin.id[:8]}) node_guid from '{pin.node_guid}' to '{final_guid}'", file=sys.stderr)
            ...
            if match:
                ...
                try:
                    ...
                    if pin_id:
                        ...
                        # Important: Use extend or |= to merge links if pin definition spans multiple lines
                        pin.linked_to_guids.extend(pin_details.get("LinkedTo", []))
                        # Remove duplicates if necessary (though unlikely for LinkedTo)
    ...
    def _finalize_node_properties(self, node: Node, property_lines: List[str]):
        ...
        if isinstance(node_comment, str):
            ...
            except Exception: node.node_comment = node_comment
        ...
        if isinstance(node, EdGraphNode_Comment):
            ...
            node.comment_color = str(node.raw_properties.get("CommentColor"))
        ...
        elif isinstance(node, K2Node_InputTouch): pass
            ...
            node.function_name = extract_member_name(func_ref) or extract_specific_type(full_property_text, MEMBER_NAME_REGEX, 1)
            ...
            if not node.component_class_path:
                class_pin = node.get_component_class_pin()
            ...
            class_ref = node.raw_properties.get("Class")
            node.subsystem_class_path = str(class_ref).strip("'\"") if class_ref else extract_specific_type(full_property_text, CLASS_PATH_REGEX, 2, "Class")
    ...
    def _resolve_links(self):
        ...
        for node_guid, node in list(self.nodes.items()):
            for pin_id, pin in list(node.pins.items()):
                ...
                for target_node_ref, target_pin_id_ref in pin.linked_to_guids:
                    ...
                    if resolved_guid_from_name:
                        ...
                        target_node = self.nodes.get(actual_target_guid)
                        # if ENABLE_PARSER_DEBUG: print(f"      Node Ref '{target_node_ref}' found in name map -> GUID '{actual_target_guid}'. Node object found: {target_node is not None}", file=sys.stderr)
                    else:
                        actual_target_guid = target_node_ref
                        target_node = self.nodes.get(actual_target_guid)
                        # if ENABLE_PARSER_DEBUG: print(f"      Node Ref '{target_node_ref}' not in name map. Treating as GUID. Node object found: {target_node is not None}", file=sys.stderr)

                    if target_node:
                        target_pin = target_node.pins.get(target_pin_id_ref)

                        if target_pin:
                            if ENABLE_PARSER_DEBUG: print(f"      Target Pin FOUND: '{target_pin.name}' ({target_pin_id_ref}) within Node '{target_node.name}'. SUCCESS.", file=sys.stderr)
                            ...
                            if ENABLE_PARSER_DEBUG:
                                print(f"      Target Pin ID '{target_pin_id_ref}' NOT FOUND within pins of target node '{target_node.name}' ({target_node.guid[:8]}). LOOKUP FAILURE.", file=sys.stderr)
                                target_pin_ids_on_node = list(target_node.pins.keys())
                                print(f"        Available Pin IDs on target node: {target_pin_ids_on_node}", file=sys.stderr)
                    else:
                        unresolved_name_lookups += 1
                        if ENABLE_PARSER_DEBUG: print(f"      Target Node NOT FOUND using ref '{target_node_ref}' (resolved GUID: '{actual_target_guid}'). NAME/GUID LOOKUP FAILURE.", file=sys.stderr)
                        if resolved_guid_from_name and resolved_guid_from_name not in self.nodes:
                             unresolved_missing_nodes += 1
                             if ENABLE_PARSER_DEBUG: print(f"        (Name lookup succeeded, but node object missing from dictionary)", file=sys.stderr)
